...
| `SyntheticL1ADDataModule` | In-memory L1-like smoke-test data and controlled Gaussian-subspace studies. | `n_features`, `n_train`, `n_val`, `n_test`, `batch_size`, `seed`, `paper_aliases`, `generator`, `anomaly_shift`, `anomaly_dim`, `reference_shift`. | Default `generator=shifted` preserves smoke-test tensors. `generator=gaussian_subspace` gives analytically tractable normal/reference/anomaly Gaussian domains. |
