...
| `check_hf_parity.py` | Check that the two physics pipelines end at the same tensors. `physics --raw-data-dir <parquet files>` surveys `data=basis` and `data=basis_hf` in turn, comparing the tensors, labels, dataset order, feature map and normalisation parameters a model would be handed; `cifar` compares the hub's CIFAR-10 against `torchvision`. Needs `PYTHONPATH=$PWD`. `--raw-cache` and `--hf-cache` build each side somewhere fresh, which a changed pipeline stage requires: neither side rebuilds a cache that already exists, so a stale one would be compared instead. |
