...
Reads the per-study CSVs produced by ``scripts/optuna/fetch_optuna_pareto.py`` (in
...
clariden -- one job per point -- via ``scripts/cluster/submit_pareto.sh``
(submitit launcher; it strips the taskset/device pinning and prepends ``-m
...
Usage (from anywhere; paths are anchored to the repo root)::
    ...
    python scripts/optuna/make_pareto_scripts.py            # write all scripts
    python scripts/optuna/make_pareto_scripts.py --dry-run  # summary only

    # generate into a scratch tree from one strategy's CSVs, for splicing
    python scripts/optuna/make_pareto_scripts.py \\
...
def _command_body(
    ...
    # The "fixed" tokens are written verbatim, unlike the Optuna params below,
    # which go through quote(). Bash leaves a non-matching bracket glob such as
    # data.image_size=[128,128] literal, so both forms work -- but quoting them
    # now would make this generator disagree with every checked-in file and
    # break splice_pareto_section.py's byte-exact self-check.
    body = list(dom["fixed"])
...
def command_lines(
    ...
    generated file feeds these same blocks through scripts/cluster/submit_pareto.sh,
...
def generate_file(
    ...
    out = [
        ...
        BANNER,
        f"# {subtitle}",
        f"# each validation strategy. Generated from notebooks/paretos/{domain}/ by",
    ...
    # Only physics blocks carry a raw-data path, and submit_pareto.sh aborts on
    # the '/path/to/...' placeholder -- so the other domains must be told to pass
    # nothing, or the documented command fails on the first job.
    submit = f"# bash scripts/cluster/submit_pareto.sh scripts/{domain}/{out_path.name}"
    if any("paths.raw_data_dir=" in f for f in DOMAINS[domain]["fixed"]):
        recipe = [
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "single-section files, which is what scripts/optuna/splice_pareto_section.py "
        "wants.",
    )
    parser.add_argument(
        "--out-root",
    ...
    default_paretos = repo / "notebooks" / "paretos"
    paretos = Path(args.paretos_dir).expanduser() if args.paretos_dir else default_paretos
