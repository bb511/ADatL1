...
one.
    ...
    python3 scripts/sweep_status.py            # table + commands
    python3 scripts/sweep_status.py --commands # commands only
...
def main():
    ...
    rows = []
    for script in sorted(glob.glob("scripts/*/run*_search.sh")):
        domain = script.split("/")[1]
        stem = os.path.basename(script)[:-3]                      # runsvdd_q99_search
        model = re.match(r"run([a-z0-9]+?)(?:_q99)?_search", stem).group(1)
        root = DB_ROOT or "logs/optuna"
        ...
        for study, (idx, target) in blocks(script).items():
            done = complete_of(db, study) if has_db else 0
    ...
    if not need:
        ...
        return 0
    if migrated:
        print("# %d study(ies) carried over from olqti sit below target; top up only if you want" % len(migrated))
        for r in migrated:
            print("#   %-9s %-8s %-24s %d/%d   (bash scripts/submit_search.sh --only %s %s hydra.sweeper.n_trials=%d)"
                  % (r["domain"], r["model"], r["study"], r["done"], r["target"],
                     r["study"], r["script"], r["target"] - r["done"]))
        print()
    print("# %d study(ies) to launch  (run from ~/adl1t_lab on clariden)" % len(need))
    for r in need:
        todo = r["target"] - r["done"]
        print("# %-9s %-8s %-24s %d/%d complete" % (r["domain"], r["model"], r["study"], r["done"], r["target"]))
        print("bash scripts/submit_search.sh --only %s %s hydra.sweeper.n_trials=%d"
              % (r["study"], r["script"], todo))
    return 0
