...
class CouplingLayer(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.scale_net[-2], nn.Linear):
            self.scale_net[-2].weight.data.fill_(0.0)
            if self.scale_net[-2].bias is not None:
                self.scale_net[-2].bias.data.fill_(0.0)
        if isinstance(self.translate_net[-1], nn.Linear):
            self.translate_net[-1].weight.data.fill_(0.0)
            if self.translate_net[-1].bias is not None:
                self.translate_net[-1].bias.data.fill_(0.0)
