...
from src.algorithms.losses.components.mmd import MMDLoss
...
class BetaVAELoss(L1ADLoss):
    ...
    def forward(
        self, z_mean: torch.Tensor, z_log_var: torch.Tensor, **kwargs
    ) -> torch.Tensor:
        ...
        if self.capacity_leadin > 0:
            capacity = min(
                self.max_capacity, self.max_capacity * self.steps / self.capacity_leadin
            )
