...
try:
    import keras
except ImportError:  # pragma: no cover - only needed for quantized checkpoints
    keras = None
...
class VICReg(L1ADLightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._maybe_build_keras_model()

        # Instantiate augmentation modules with different rngs
        self.feat_blurs = {}
        feature_blur_1 = copy.deepcopy(feature_blur) if feature_blur else torch.nn.Identity()
        feature_blur_2 = copy.deepcopy(feature_blur) if feature_blur else torch.nn.Identity()
        if hasattr(feature_blur_1, "rng"):
            feature_blur_1.rng.set_seed(self.seed)
        if hasattr(feature_blur_2, "rng"):
        ...
        self.obj_masks = {}
        object_mask_1 = copy.deepcopy(object_mask) if object_mask else torch.nn.Identity()
        object_mask_2 = copy.deepcopy(object_mask) if object_mask else torch.nn.Identity()
        if hasattr(object_mask_1, "rng"):
            object_mask_1.rng.set_seed(self.seed)
        if hasattr(object_mask_2, "rng"):
    ...
    def model_step(self, batch: Tuple[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        add_loss = loss_total.new_tensor(0.0)
        if hasattr(self.model, "losses") and len(self.model.losses) > 0:
            add_loss = add_loss + torch.stack(
                [self._to_loss_device(l, loss_total) for l in self.model.losses]
            ).sum()
    ...
    def _load_weights_from_lightning_ckpt(
        ...
        keras_model = getattr(keras, "Model", None) if keras is not None else None

        if isinstance(model, torch.nn.Module) and (
            keras_model is None or not isinstance(model, keras_model)
        ):
            model.load_state_dict(sub_sd, strict=True)
            return model

        if keras_model is None or not isinstance(model, keras_model):
    ...
    def _maybe_build_keras_model(self) -> None:
        """Build Keras/HGQ backbones early so Lightning sees their torch parameters."""
        if not hasattr(self.model, "trainable_variables"):
            return
        if len(getattr(self.model, "trainable_variables", [])) > 0:
            return
