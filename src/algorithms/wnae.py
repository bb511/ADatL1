...
from src.algorithms.components.mcmc import SampleBuffer
    ...
    def model_step(self, batch: Tuple[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.training:
            ...
            energy_neg = self.loss.losses.reconstruction(
                target=x_neg, reco=reconstruction_neg
            )
        else:
            # Evaluation uses deterministic reconstruction energy as the anomaly score.
        ...
        return {
            "loss": loss_total,
            "loss/total/full": loss_total.detach().expand(x_pos.shape[0]),
            "loss/reconstruction": loss_reco.detach(),
            "loss/wasserstein": loss_wasserstein.detach(),
            "loss/nae": loss_nae.detach(),
            "ascore/full": energy_pos.detach(),
            "reconstructed_data": reconstruction_pos.detach(),
        }
