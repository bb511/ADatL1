...
class ResidualOASStateCallback(Callback):
    """Fit AE residual OAS state on training normals before each validation.

    The clean pass uses the final weights of the epoch. Intervention labels and validation scores
    ...
    def on_validation_epoch_start(self, trainer, pl_module):
        ...
        if int(getattr(trainer, "world_size", 1)) != 1:
            raise RuntimeError("ResidualOASStateCallback currently requires a single process.")
