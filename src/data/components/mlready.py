...
class L1DataMLReady:
    ...
    def _prepare_maindata(self):
        ...
        self._cache_l1bit(obj_paths, self.cache_folder, itrain, "train")
        self._cache_l1bit(obj_paths, self.cache_folder, ivalid, "valid")
        self._cache_l1bit(obj_paths, self.cache_folder, itest, "test")
    ...
    def _cache_l1bit(
        self, obj_paths: list[Path], cache_dir: Path, idxs: list, type: str
    ):
        """Cache the training data l1bit.

        This is used later down the pipeline to compute things such as the pure rate,
        i.e., how much better our algorithm is compared to the standard trigger one.
        """
        if not "seeds" in obj_paths.keys():
            log.warn("seeds object is not in the processed data. Skipping l1bit...")
            log.warn("It will not be possible to compute pure rates etc.")
            return

        folder = cache_dir / type
        folder.mkdir(parents=True, exist_ok=True)
        cache_file = folder / f"L1bit.parquet"
        if self._check_data_exists(cache_file):
            if self.verbose:
                log.info(Fore.YELLOW + f"Found l1bit for {folder} data.")
            return

        file_paths = obj_paths["seeds"]
        obj_data = pyarrow.dataset.dataset(file_paths, format="parquet")

        events = pyarrow.array(idxs)
        data = ak.from_arrow(obj_data.take(events))
        data = data[["L1bit"]]

        ak.to_parquet(data, cache_file)
        log.info(Fore.GREEN + f"Cached l1bit {type} data at: {cache_file}")
    ...
    def _cache_aux(self, dataset_path: Path):
        ...
        obj_paths = self._get_files_per_object(dataset_path, level=1)
        ivalid, itest = self._compute_aux_split(dataset_path)

        for obj_name, obj_path in obj_paths.items():
            if obj_name not in self.select_feats.keys():
                continue
        ...
        self._cache_l1bit(obj_paths, cache_dir, ivalid, "valid")
        self._cache_l1bit(obj_paths, cache_dir, itest, "test")
    ...
    def _get_files_per_object(self, processed_path: Path, level: int = 2) -> dict:
        """Get path to parquet files per object in the processed data folder.

        :param processed_path: Path that points to a dir or parent dir where the
            parquet files are.
        :param level: Int that points how far down from processed_path the script
            should look to find the parquet files. This is useful for gathering
        ...
        for file_path in sorted(
            ...
            if len(file_path.relative_to(processed_path).parents) == level:
