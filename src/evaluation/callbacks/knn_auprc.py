...
class KNNAUPRC(Callback):
    ...
    def on_test_start(self, trainer, pl_module):
        ...
        self.reference_dataset = self._reference_dataset(dset_names, "main_test")
    ...
    def on_test_epoch_start(self, trainer, pl_module):
        ...
        for ds_name in dset_names:
            if ds_name == self.reference_dataset or ds_name in self.skip_ds:
    ...
    def on_test_batch_end(
        ...
        if dataset_name == self.reference_dataset:
    ...
    @staticmethod
    def _reference_dataset(dset_names: list[str], legacy_name: str) -> str:
        if legacy_name in dset_names:
            return legacy_name
        if "normal" in dset_names:
            return "normal"
        raise ValueError(
            f"KNN AUPRC needs a normal test dataset. Available: {dset_names}"
        )
    ...
    def _store_summary(self, auprcs_per_dataset: dict, ckpt_name: str):
        ...
        """
        ckpt_auprcs = list(auprcs_per_dataset.values())
        ckpt_auprcs_mean = np.mean(ckpt_auprcs)
        ckpt_auprcs_var = np.var(ckpt_auprcs)

        eps = 1e-14
        summary_metric = ckpt_auprcs_mean ** 2 / (ckpt_auprcs_var + eps)

        ckpt_ds_name = utils.misc.get_ckpt_ds_name(ckpt_name)
        self.auprcs_summary[ckpt_ds_name] = summary_metric

    def plot_summary(self, trainer, root_folder: Path):
        """Plot the summary metrics accummulated in eff_summary."""
        plot_folder = root_folder / "plots" / "knn_auprcs_summary"
        plot_folder.mkdir(parents=True, exist_ok=True)
        self._cache_summary(plot_folder)

        xlabel = f"mu^2/sigma ds knn_auprcs"
        ylabel = " "
        horizontal_bar.plot_yright(
            self.auprcs_summary,
            self.auprcs_summary,
        ...
        utils.mlflow.log_plots_to_mlflow(trainer, None, "knn_auprcs", plot_folder)
    ...
    def get_optimized_metric(self, ckpt_ds: str = None):
        ...
        Here, if a ckpt_ds is given, the self.auprcs_summary is returned for the
        specified ckpt_ds. Otherwise, find the ckpt_ds for which the summary value
        is maximal. For what this summary value is, check the _store_summary method.
        """
        if ckpt_ds:
            return ckpt_ds, self.auprcs_summary[ckpt_ds]

        max_ckpt_ds = max(self.auprcs_summary, key=self.auprcs_summary.get)
        optimized_metric = self.auprcs_summary[max_ckpt_ds]
        return max_ckpt_ds, optimized_metric
