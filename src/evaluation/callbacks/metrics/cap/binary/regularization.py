...
def threshold(
    prob: Tensor, maxcount: int, mode: Literal["max", "mean", "zero"] = "max", **kwargs
) -> Tensor:
    """
    ...
    **kwargs,
    ...
    if mode == "sigmoid":
        ...
        if below_threshold.any():
            ...
            regularized_prob[below_threshold] = prob[below_threshold] * torch.exp(
                -distance
            )
        ...
        if below_threshold.any():
            ...
            normalized = (prob[below_threshold] - min_val) / (
                threshold_value - min_val + 1e-8
            )
            regularized_prob = prob.clone()
...
def percentile(
    ...
    **kwargs,
    ...
    if below_threshold.any():
        ...
        normalized = (below_values - below_values.min()) / (
            below_values.max() - below_values.min() + 1e-8
        )
    ...
    if above_threshold.any():
        ...
        normalized = (above_values - above_values.min()) / (
            above_values.max() - above_values.min() + 1e-8
        )
        regularized_prob[above_threshold] = enhancement_percentile + normalized * (
            1.0 - enhancement_percentile
        )
