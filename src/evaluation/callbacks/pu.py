...
class BkgRatePileup(Callback):
    ...
    def _get_pileup_data(self, datamodule: LightningDataModule):
        ...
        test_data = datamodule.get_extra(normalizer, extra_feats, "test", flag)
    ...
    def on_test_batch_end(
        ...
        if self.dataset_name == "main_test" and batch_idx < self.total_batches / 2:
            ...
            self._compute_batch_rate(outputs, batch_idx)
            self._accumulate_singlenetr_output(outputs, batch_idx)
    ...
    def _accumulate_maintest_output(self, outputs: dict, batch_idx: int):
        ...
        if (batch_idx + 1) >= self.total_batches / 2:
    ...
    def _accumulate_singlenetr_output(self, outputs: dict, batch_idx: int):
        ...
        """
        batch_output = outputs[self.metric_name]
        self.singnetr_score_data.append(batch_output)

        if batch_idx == self.total_batches - 1:
            self.singnetr_score_data = torch.cat(self.singnetr_score_data, dim=0)
    ...
    def _accumulate_mtheldout_output(self, outputs: dict, batch_idx: int):
        ...
        """
        batch_output = outputs[self.metric_name]
        self.maintest_heldout_score_data.append(batch_output)
        if batch_idx == self.total_batches - 1:
            self.maintest_heldout_score_data = torch.cat(
                self.maintest_heldout_score_data, dim=0
            )

    def _get_pu_batch(self, dataset, batch_idx: int) -> torch.Tensor:
        # dataset is your L1ADDataset (IterableDataset) that holds PU in .data
        bs = dataset.batch_size
        s = batch_idx * bs
        e = min(s + bs, dataset.n)
        return dataset.data[s:e]
    ...
    def _compute_batch_rate(self, outputs: dict, batch_idx: int):
        ...
        pus = pus.round().int().to(self.device).view(-1)
        pus, inv = pus.unique(sorted=True, return_inverse=True)

        scores_by_pu = {
            int(pu.item()): outputs[self.metric_name]
            .detach()
            .to(self.device)
            .float()[inv == i]
        ...
        for target_rate in self.target_rates:
            ...
            for pu, scores in scores_by_pu.items():
                if not pu in self.rates_per_pu[rate_name].keys():
                    self._initialize_rate_metric(target_rate, pu)
    ...
    def on_test_epoch_end(self, trainer, pl_module) -> None:
        ...
        for rate_name, pu_rates in self.rates_per_pu.items():
            ...
            pu_rates = self._compute_rates(
                pu_rates, dataset_name, target_rate, plot_folder
            )
    ...
    def _compute_rates(
        self, pu_rates: dict, dataset_name: str, target_rate, plot_folder
    ):
        ...
        if "main_test" == dataset_name:
            ...
            raise ValueError("Dataset name is weird.")
    ...
    def _plot_score(self, data, dataset_name, threshold, target_rate, plot_folder):
        # Convert tensor to CPU numpy array (safe even if already on CPU)
        ...
        ax.axvline(
            x=threshold, linestyle="--", linewidth=2, label=f"Threshold = {threshold}"
        )

        ax.set_xlabel(f"Score @ {target_rate} kHz")
        ax.set_ylabel("Count")
        ax.set_yscale("log")
        ax.legend()
        ...
        filename = sanitize_filename(f"{dataset_name}_{target_rate}")
        filename = filename.replace(" ", "_")

        fig.savefig(plot_folder / f"{filename}.jpg", bbox_inches="tight")
        plt.close(fig)
