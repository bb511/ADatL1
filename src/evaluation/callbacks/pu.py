...
class BkgRatePileup(Callback):
    ...
    def __init__(
        ...
        self.bc_rate = bc_rate if bc_rate is not None else base_rate
        self.metric_name = metric_name if metric_name is not None else output_name
        if self.bc_rate is None or self.metric_name is None:
            raise ValueError(
                "BkgRatePileup requires bc_rate/base_rate and metric_name/output_name."
            )
    ...
    def on_test_start(self, trainer, pl_module):
        ...
        if first_test_dset_key not in {"main_test", "normal"}:
            raise ValueError(
                "PU callback requires main_test or normal first in the data dict!"
            )
        self.reference_dataset = first_test_dset_key
    ...
    def on_test_batch_end(
        ...
        if self.dataset_name == self.reference_dataset:
    ...
    def _accumulate_maintest_output(self, outputs: dict, batch_idx: int):
        ...
        """
        batch_output = outputs[self.metric_name].detach().to("cpu").float()
        self.maintest_score_data.append(batch_output)

        pu, _ = self.pu_per_ds[self.reference_dataset][batch_idx]
    ...
    def _compute_maintest_rate(self):
        ...
        for target_rate in self.target_rates:
            ...
            for pu, scores in scores_by_pu.items():
                rate = AnomalyRate(target_rate, self.bc_rate).to("cpu")
