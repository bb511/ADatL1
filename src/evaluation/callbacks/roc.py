...
class ROCs(Callback):
    ...
    def on_test_epoch_start(self, trainer, pl_module):
        ...
        for ds_name in dset_names:
            if ds_name == self.reference_dataset:
    ...
    def _reference_dataset(dset_names: list[str], legacy_name: str) -> str:
        ...
        if "normal" in dset_names:
            return "normal"
        raise ValueError(f"ROC callback needs a normal test dataset. Available: {dset_names}")
    ...
    def _convert2numpy(self, arr: tuple[torch.Tensor] | torch.Tensor):
        ...
        if isinstance(arr, tuple):
            arr = tuple(tens.detach().cpu().numpy() for tens in arr)
