...
class axo_threshold_manager:
    def __init__(self, model, config):
        ...
        self.input_quantizer = quantized_bits(
            precision[0], precision[1], alpha=self.config["alpha"]
        )
        self.get_threshold()
        self.get_axo_score()
    ...
    def get_threshold(self):
        ...
        latent_axo_qk = self.model.predict(
            self.input_quantizer(x_test), batch_size=120000
        )
        y_axo_qk = np.sum(latent_axo_qk**2, axis=1)

        threshold = {}
        for target_rate in self.target_rate:
            threshold[str(target_rate)] = np.percentile(
                y_axo_qk, 100 - (target_rate / self.bc_rate_khz) * 100
            )
    ...
    def get_axo_score(self):
        ...
        for thres in self.threshold.keys():
            ...
            for signal in signal_names:
                ...
                latent_axo_qk = self.model.predict(
                    self.input_quantizer(signal_data),
