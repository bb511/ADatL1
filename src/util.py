...
def mutual_information_bernoulli_loss(
        y_true, # The sensitive attribute (e.g., pileup count), rename to S
        y_pred, # The model output on which MI is computed, (e.g., probabilities of Bernoulli latent units), rename to L_n
        use_quantized_sigmoid=False, # Whether to use a quantized sigmoid function for computing probabilities in get_h_bernoulli.
        bits_bernoulli_sigmoid=8 # Number of bits for the quantized sigmoid if use_quantized_sigmoid is True. Higher bits means finer quantization.
        ): 
    """
    I(x;y)  = H(x)   - H(x|y)
            = H(L_n) - H(L_n|s)
    ...
    :return: The loss
    """
    # Bernoully because signoid turns each neuron into 0 or 1 with a certain probability

    def get_h_bernoulli(tensor) -> float: # Computes Bernoully entropy of a tensor, expected input: tensor.shape = [batch, latent_dim]
        def get_theta(x): # Converts raw latent values into Bernoulli probabilities
            # alpha = None # disables the adaptive standard deviation scaling branch below. this means std = 1.0 always
            temperature = 6.0 # Sharpens the sigmoid. Higher value means sharper sigmoid (more step like), which means probabilities closer to 0 or 1, which means higher MI. Is a hyperparameter.

            # if isinstance(alpha, str): # Checks if alpha is one of the allowed values
            #     assert alpha in ['auto', 'auto_po2']

            # if isinstance(alpha, str):
            #     len_axis = len(x.shape)

            #     if len_axis > 1: # Would decide across which axes to compute the standard deviation
                                    # For dense latent vectors, this would likely compute statistics across the batch axis
            ...
            #     std = ops.std(x, axis=axis, keepdims=True) + keras.backend.epsilon() # Normalize the input by its std
            # else:
            #     std = 1.0

            std = 1.0

            if use_quantized_sigmoid:
                # quantized sigmoid
                # quantized sigmoid approximates sigmoid with finite precision, controlled by bits_bernoulli_sigmoid.
            ...
            return p # Returns probabilities in [0, 1] for each latent unit

        def log2(x) -> tf.Tensor: # Computes log base 2 of x, with numerical stability for log(0)
            numerator:any = tf.math.log(x + 1e-20)
            denominator: tf.Tensor = tf.math.log(tf.constant(2, dtype=numerator.dtype))
            return numerator / denominator

        # ESTIMATE BERNOULLI PROBABILITY PER LATENT UNIT
        # get_theta maps latent activations to Bernoulli probabilities
        # Then average over the batch with tf.reduce_mean(..., axis=0)
        theta = tf.reduce_mean(get_theta(tensor), axis=0)

        # COMPUTE BERNOULLI ENTROPY
        # For each latent unit, compute the Bernoulli entropy using the formula H = -(1-theta) * log2(1-theta) - theta * log2(theta)
        # Then sum over all latent units with tf.reduce_sum
        return tf.reduce_sum(-(1 - theta) * log2(1 - theta) - theta * log2(theta))

    def compute_for_value(value, y_true, y_pred, get_h_bernoulli): # Computes the conditional entropy for one value of the sensitive attribute (e.g., s=0 or s=1)

        if tf.shape(y_true).shape[0] == 1: # Handles the case if y_true is 1D
            y_filter = tf.where(y_true == value)
        else: # Handles the case if y_true is 2D
            y_filter = tf.where(y_true[:, 0] == value)[:, 0]

        y_i = tf.gather(y_pred, indices=y_filter) # Selects the latent representations belonging to the current sensitive group, e.g. for S=0 selects all L_n for which the corresponding S is 0
        H_L_n_si = get_h_bernoulli(y_i) # Computes H(L | S), is entropy of the latent representation inside one sensitive group

        # TODO: However tf.cast(...) = 0
        # Better: cnt_i = tf.cast(tf.shape(y_i)[0], tf.float64) + 1e-16
        cnt_i = tf.shape(y_i)[0] + tf.cast(1e-16, dtype=tf.int32)  # Counts the number of samples in this group # number of repr with index i
        norm_si = cnt_i / tf.shape(y_pred)[0] # Computes the emirical probability for this goup, e.g. P(S = value) = number of samples with S=value / batch_size TODO: cast both sides as float to avoid integer division
        return H_L_n_si, norm_si

    y_true = tf.cast(y_true, tf.int32) # Converts the sensitive lablels to integers
    y_pred = tf.cast(y_pred, tf.float64) # Converts the latent outputs to float64
    H_L_n = get_h_bernoulli(y_pred) # Computes the marginal entropy H(L)

    unique_y_true, _ = tf.unique(y_true) # Finds all sensitive attribute values that occur in the current batch

    def compute_per_value(v) -> tuple[any, tf.Tensor]: # a wrapper function so that tf.map_fn can call compute_for_value for each unique sensitive value.
        return compute_for_value(v, y_true, y_pred, get_h_bernoulli)


    # Applies compute_for_value to every sensitive class/bin in the batch

    results = tf.map_fn(compute_per_value, unique_y_true, fn_output_signature=(tf.float64, tf.float64))

    H_L_n_s = tf.convert_to_tensor(results[0]) # Storing the conditional entropies
    norm_s = tf.convert_to_tensor(results[1]) # Storing the empilical class probabilities

    # COMPUTE MUTUAL INFORMATION
    MI = H_L_n - tf.reduce_sum(tf.math.multiply(norm_s, H_L_n_s)) # Computes I(L; S) = H(L) - H(L | S), where H(L | S) = sum_s P(S=s) H(L | S=s)
