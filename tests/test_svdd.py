...
def test_svdd_rejects_biased_encoder() -> None:
    model = _model(encoder=_encoder(bias=True))
    with pytest.raises(ValueError, match="biased layers"):
        model._validate_encoder_architecture()
...
def test_pretrained_encoder_loader_is_strict(tmp_path) -> None:
    source = _encoder()
    with torch.no_grad():
    ...
    torch.save(
        {"state_dict": {f"encoder.{key}": value for key, value in source.state_dict().items()}},
        checkpoint,
    )

    target = _model(pretrained_encoder_ckpt=str(checkpoint))
    target._load_pretrained_encoder()
